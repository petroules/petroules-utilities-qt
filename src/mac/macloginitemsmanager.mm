...
public:
    ...
    {
        // This will retrieve the path for the application, i.e. /Applications/Silverlock.app
...
bool MacLoginItemsManager::appendRunningApplication() const
{
    if (this->d->loginItems)
    {
        // Add Silverlock to the list
        LSSharedFileListItemRef item = LSSharedFileListInsertItemURL(this->d->loginItems, kLSSharedFileListItemLast, NULL, NULL, this->d->runningApplicationUrl(), NULL, NULL);
        if (item)
        {
            CFRelease(item);
            return true;
