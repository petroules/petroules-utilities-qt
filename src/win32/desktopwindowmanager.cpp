...
typedef HRESULT (WINAPI *PtrDwmEnableComposition)(UINT uCompositionAction);

static PtrDwmIsCompositionEnabled pDwmIsCompositionEnabled = 0;
static PtrDwmEnableBlurBehindWindow pDwmEnableBlurBehindWindow = 0;
static PtrDwmExtendFrameIntoClientArea pDwmExtendFrameIntoClientArea = 0;
static PtrDwmGetColorizationColor pDwmGetColorizationColor = 0;
static PtrDwmEnableComposition pDwmEnableComposition = 0;
...
static bool resolveLibs()
{
    if (!pDwmIsCompositionEnabled)
    {
        ...
        pDwmEnableComposition = (PtrDwmEnableComposition)dwmLib.resolve("DwmEnableComposition");
    }
...
/*!
    Returns a value indicating whether Windows DWM composition is currently enabled on the system.
 ...
 */
bool DesktopWindowManager::isCompositionEnabled()
{
#ifdef Q_WS_WIN
    if (resolveLibs())
    {
...
/*!
    Enables or disables Windows DWM composition on the system.
 */
bool DesktopWindowManager::enableComposition(bool enable)
{
#ifdef Q_WS_WIN
    if (resolveLibs())
    {
        return SUCCEEDED(pDwmEnableComposition(enable ? DWM_EC_ENABLECOMPOSITION : DWM_EC_DISABLECOMPOSITION));
    }
#else
    Q_UNUSED(enable);
#endif
...
/*!
    ...
    \a enable true to enable blur-behind, false to disable it
 */
 ...
 */
QColor DesktopWindowManager::colorizationColor()
{
...
#ifdef Q_WS_WIN
WindowNotifier* DesktopWindowManager::windowNotifier()
{
...
bool WindowNotifier::winEvent(MSG *message, long *result)
{
    ...
    {
        bool compositionEnabled = DesktopWindowManager::isCompositionEnabled();
        ...
        foreach (QWidget * widget, widgets)
        {
            if (widget)
            {
                ...
                {
                    DesktopWindowManager::extendFrameIntoClientArea(widget);
                }
